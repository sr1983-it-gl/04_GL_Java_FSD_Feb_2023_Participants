-- Display one customer from each product category who purchased a maximum number of cylinders with Connection_Type, Cust_Id, Name and Quantity purchased.
...
rank() over(partition by C.Connection_Type order by P.no_of_cylinders desc) as
rnk
...
(select Cust_Id, sum(Quantity) as no_of_cylinders from orders where status = 'ordered'
group by cust_Id)
as P on P.Cust_Id = C.Id) as S where R.cyl = S.no_of_cylinders and R.connection_type
= S.connection_type;
